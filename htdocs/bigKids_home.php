...
<section class="regular slider">
  ...
  <?php
				$query = "SELECT * FROM ssubcategory where typeID=3 and catID=1  ";
...
<h4 class="text-info"><?php echo $row["Title"]; ?></h4>
  ...
  <?php
				$query = "SELECT * FROM ssubcategory where typeID=3 and catID=2";
				$result = mysqli_query($db, $query);
				if(mysqli_num_rows($result) > 0)
				{
					while($row = mysqli_fetch_array($result))
...
<h4 class="text-info"><?php echo $row["Title"]; ?></h4>
  ...
  <section class="regular slider" >
  <?php
				$query = "SELECT * FROM ssubcategory where typeID=3 and catID=3  ";
				$result = mysqli_query($db, $query);
				if(mysqli_num_rows($result) > 0)
				{
					while($row = mysqli_fetch_array($result))
...
<h4 class="text-info"><?php echo $row["Title"]; ?></h4>
  ...
  <?php
				$query = "SELECT * FROM ssubcategory where typeID=3 and catID= 4 ";
				$result = mysqli_query($db, $query);
				if(mysqli_num_rows($result) > 0)
				{
					while($row = mysqli_fetch_array($result))
